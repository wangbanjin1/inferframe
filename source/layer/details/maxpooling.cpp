...
#pragma omp parallel for num_threads(batch)
    for (uint32_t i = 0; i < batch; ++i) {
        ...
        if (padding_h_ > 0 || padding_w_ > 0) {
            ...
            input_data_ = TensorPadding(input_data, {padding_h_, padding_h_, padding_w_, padding_w_},
...
ParseParameterAttrStatus MaxPoolingLayer::GetInstance(const std::shared_ptr<RuntimeOperator>& op,
    ...
    max_layer =
        std::make_shared<MaxPoolingLayer>(padding_values.at(0), padding_values.at(1), kernel_values.at(0),
