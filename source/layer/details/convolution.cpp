    ...
    if (groups_ == 1) {
        ...
        for (uint32_t k = 0; k < kernel_count_group; ++k) {
            ...
            for (uint32_t ic = 0; ic < kernel->channels(); ++ic) {
                memcpy(kernel_matrix_c.memptr() + row_len * ic, kernel->slice(ic).memptr(),
...
#pragma omp parallel for schedule(dynamic)
            for (uint32_t k = 0; k < kernel_count_group; ++k) {
                arma::fmat output(output_tensor->slice(k + g * kernel_count_group).memptr(), output_h,
                                  output_w, false, true);
                if (groups_ == 1) {
...
ParseParameterAttrStatus ConvolutionLayer::GetInstance(const std::shared_ptr<RuntimeOperator>& op,
    ...
    conv_layer = std::make_shared<ConvolutionLayer>(
        out_channel->value, in_channel->value, kernels.at(0), kernels.at(1), paddings.at(0), paddings.at(1),
        strides.at(0), strides.at(1), groups->value, use_bias->value);
