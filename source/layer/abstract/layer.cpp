...
InferStatus Layer::Forward() {
    ...
    // 准备节点layer计算所需要的输入
    const std::vector<std::shared_ptr<RuntimeOperand>>& input_operand_datas = runtime_operator_sp->input_operands_seq;
    // layer的输入
    ...
    CHECK(!layer_input_datas.empty()) << runtime_operator_sp->name << " Layer input data is empty";
    CHECK(runtime_operator_sp->output_operands != nullptr && !runtime_operator_sp->output_operands->datas.empty()) << "Layer output data is empty";
    // 执行operator当中的layer计算过程
    // layer的计算结果存放在current_op->output_operands->datas中
    InferStatus status = runtime_operator_sp->layer->Forward(layer_input_datas, runtime_operator_sp->output_operands->datas);
    return status;
}
